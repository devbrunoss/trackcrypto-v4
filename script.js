...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  // Cache simples de preços — evita bater no limite de requisições da
  // CoinGecko quando a página recarrega várias vezes seguidas.
  const priceCache = {};
...
`;
  ...
  async function updateCryptoData() {
    try {
      ...
      // Usa o cache se ainda estiver "fresco" (evita bater no limite da API
      // quando a página recarrega várias vezes em poucos segundos)
      const cached = priceCache[cryptoId];
      if (cached && (Date.now() - cached.timestamp) < PRICE_CACHE_TTL) {
        console.log(`Usando cache para: ${cryptoId}`);
        renderCryptoData(cached.cryptoData, cached.detailData);
        return;
      }
      ...
      // Buscar informações adicionais (nome, símbolo, ranking, supply) numa
      // ÚNICA chamada — antes isso era buscado 2x no mesmo endpoint.
      let detailData = null;
      try {
      ...
      // Guarda no cache antes de renderizar
      priceCache[cryptoId] = { timestamp: Date.now(), cryptoData, detailData };

      renderCryptoData(cryptoData, detailData);
  ...
  // Atualiza a UI com os dados recebidos (da API ou do cache)
  function renderCryptoData(cryptoData, detailData) {
    const cryptoId = elements.cryptoSelect.value;

    // Nome e símbolo
    if (detailData && detailData.name && detailData.symbol) {
      elements.cryptoName.textContent = `${detailData.name} (${detailData.symbol.toUpperCase()})`;
    } else {
    ...
    // Ranking e supply (vêm do detailData)
    if (detailData) {
